...
//--------------------read clusterlist from cluster.out and eci from eci.out-------------------------------
  ...
  clusterlist=wfun.getClusterlist("clusters.out");
  ...
  while (!logf.eof()){                    
     ...
     if (row>2*binSize+4 and row<3*binSize+5) {
        ...
        while (strings[column] != NULL){ 
          ...
          if (column==3) {count[row-2*binSize-5]=atoi(strings[column]);cout<<count[row-2*binSize-5]<<endl;}
          //column +=1; cout<<column<<" "<<atof(p[column])<<"\n";
          strings[++column] = strtok(NULL, split);} //cout <<count[row-2*binNum-5]<<endl; }
        for (int i=0;i<numEle*numEle;i++){sro[row-2*binSize-5][column]=sro[row-2*binSize-5][column]*count[row-2*binSize-5];}
     }
   } 
...
E0=wfun.cal_etot(latticefilename,strfilename,clusterfile,ecifile);
